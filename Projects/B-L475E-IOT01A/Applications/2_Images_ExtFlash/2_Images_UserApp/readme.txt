...
2. Pressing 2 allows to test protections.
   ...
   - TAMPER test (#7): causes a reset if a tamper event is detected. In order to generate a tamper event, user has to
     connect PA0 (CN3.2) to GND (It may be enough to put your finger close to PA0 (CN3.2)).
...
It allows to download a new firmware image in a specific area (DWL_SLOT #1, DWL_SLOT #2, DWL_SLOT #3).
Installation is done on request after one or several downloads.
Be careful, the firmware should have been built for execution in another active slot than ACTIVE_SLOT #1.

For example, to execute a firmware in the slot ACTIVE_SLOT #2, following changes are needed :
   - modify the postbuild command of the UserApp to select the correct magic in the firmware header (SFU1, SFU2, SFU3) :
     replace, cmd /C "cmd /C $PROJ_DIR$\..\..\2_Images_SECoreBin\EWARM\PostBuild.bat "$PROJ_DIR$" "$TARGET_PATH$" "$PROJ_DIR$\UserApp.bin" 1 1"
     by,      cmd /C "cmd /C $PROJ_DIR$\..\..\2_Images_SECoreBin\EWARM\PostBuild.bat "$PROJ_DIR$" "$TARGET_PATH$" "$PROJ_DIR$\UserApp.bin" 2 1"
   - select the correct execution area in the user application mapping file :
     replace, __ICFEDIT_SLOT_Active_1_start__ and __ICFEDIT_SLOT_Active_1_end__
     by,      __ICFEDIT_SLOT_Active_2_start__ and __ICFEDIT_SLOT_Active_2_end__
...
Note1 : The slot active #3 is not configured in this example.
