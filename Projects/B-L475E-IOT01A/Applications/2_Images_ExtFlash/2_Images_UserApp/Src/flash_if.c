...
HAL_StatusTypeDef FLASH_EXT_If_Write(void  *pDestination, const void *pSource, uint32_t uLength)
{
  ...
  if (BSP_QSPI_Write((uint8_t *) pSource, (uint32_t) pDestination - EXTERNAL_FLASH_ADDRESS, uLength) != QSPI_OK)
  {
    return HAL_ERROR;
  }
  else
...
HAL_StatusTypeDef FLASH_EXT_If_Read(void *pDestination, const void *pSource, uint32_t uLength)
{
  ...
  if (BSP_QSPI_Read((uint8_t *) pDestination, (uint32_t) pSource - EXTERNAL_FLASH_ADDRESS, uLength) != QSPI_OK)
  {
    return HAL_ERROR;
  }
  else
