...
STM32H750B-DK board  provides 128 Kbytes of internal flash in a single sector and 128Mbytes of external flash. Active
slot as well as download slot are mapped in external flash. As a consequence, some features are not available:
   - Firmware confidentiality cannot be ensured as the user application is decrypted before its execution.
   - WRP protection cannot be set on internal flash since anti-rollback mechanism is based on counters stored in
     internal flash. SBSFU immutable code is garranty with HDP protection during user application execution but cannot
     be guarantee during boot execution.
   - Loader functionality provided in user application must be executed from RAM.

In this example slots are located in external flash. OTFDEC (On The Fly Decryption) is configured to execute the
encrypted (AES CTR) user application from external flash.
...
@par Directory contents
   ...
   - 2_Images_SBSFU/Core/Inc/stm32h750b_discovery_conf.h      BSP configuration file
...
@par Hardware and Software environment
   ...
   - This example has been tested with STM32H750B-DK board and can be easily tailored to any other supported device and
     development board.
...
2. Before loading SBSFU image into the target, please ensure with STM32CubeProgammer available on www.st.com that the
   ...
   - External memory has been erased (see the STM32CubeProgrammer User Manual for more details about
     the external flash memory programming)
