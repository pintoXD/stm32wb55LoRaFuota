...
2. Pressing 2 allows to test protections.
   ...
   - TAMPER test (#4): causes a reset if a tamper event is detected. In order to generate a tamper event, user has to
     connect PA0 (CN8.4) to GND (It may be enough to put your finger close to PA0 (CN8.4)).
   - CORRUPT IMAGE test (#4): causes a signature verification failure of the selected firmware image at next boot.
