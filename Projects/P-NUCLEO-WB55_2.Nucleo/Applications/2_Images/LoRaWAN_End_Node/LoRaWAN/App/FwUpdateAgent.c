...
#include "se_def_metadata.h"        /* the struct definition is dependent of the crypto scheme used*/
...
void FwUpdateAgentRun(void)
{
  ...
  if (FwUpdateAgentGetDownloadAreaInfo(&fw_image_dwl_area) != HAL_ERROR)
  {
    ...
    /* System Reboot*/
    APP_LOG(TS_OFF, VLEVEL_M, "  -- Image correctly downloaded - reboot\r\n\n");
    HAL_Delay(1000U);
    NVIC_SystemReset();
  }
  if (ret != HAL_OK)
  {
    APP_LOG(TS_OFF, VLEVEL_M, "  --  Operation Failed  \r\n");
  }
}
...
HAL_StatusTypeDef FwUpdateAgentDataTransferFromRamToFlash(uint8_t *pData, uint32_t uFlashDestination, uint32_t uSize)
{
  ...
  /* Clear Download area*/
  if (FlashMemHandlerFct.Erase_Size((void *)SLOT_ACTIVE_1_START, (SLOT_ACTIVE_1_END - SLOT_ACTIVE_1_START)) == HAL_OK)
  {
