...
static void tkms_app_print_menu(void)
{
  ...
  (void)printf("  TKMS - Tests ECDH Key Gen + Diffie-Hellman        --------------------- 6\r\n\n");
  (void)printf("  TKMS - Tests Ext. Token Sign Verify               --------------------- a\r\n\n");
    ...
    if (COM_Receive(&key, 1U, RX_TIMEOUT) == HAL_OK)
    {
      switch (key)  /* User pressed key */
      {
        case '0' :
          ...
          tests_success += (ret_status == CKR_OK) ? 1U : 0U;
          INVOKE_SCHEDULE_NEEDS();
          ret_status = tkms_app_ECDH_generate_derive_key();
          (void)printf("ECDH key gen test %s\r\n", (ret_status == CKR_OK) ? "SUCCESSFUL" : "FAILED");
          tests_executed++;
          tests_success += (ret_status == CKR_OK) ? 1U : 0U;
          INVOKE_SCHEDULE_NEEDS();
          ret_status = tkms_app_ext_token_sign_verify_ecdsa();
          (void)printf("Ext Token test %s\r\n", (ret_status == CKR_OK) ? "SUCCESSFUL" : "FAILED");
          tests_executed++;
          tests_success += (ret_status == CKR_OK) ? 1U : 0U;
          INVOKE_SCHEDULE_NEEDS();
          /* Report final status */
        ...
        case '5':
          ...
          tkms_app_print_menu();
          break;
        /* 6 = Tests ECDH - key generation + Diffie-Hellman key derivation*/
        case '6' :
          INVOKE_SCHEDULE_NEEDS();
          ret_status = tkms_app_ECDH_generate_derive_key();
          (void)printf("ECDH key gen test %s\r\n", (ret_status == CKR_OK) ? "SUCCESSFUL" : "FAILED");
