...
SFU_ErrorStatus SFU_LL_SECU_SetProtectionMPU(uint8_t uStep)
{
  ...
  /* Compute the number of 16Kbytes sub-regions required to cover the executable size in Region 1 (up to 8 maximum).
    ...
    */
  region1_first_32kb_sub_region_disabled = sbsfu_code_size >> 15U ; /* shifting by 15 to divide by 32k */

  /* Compute the remainder of the code size divided by 16k */
  region2_first_4kb_sub_region_disabled = sbsfu_code_size & 32767U;

  /* Compute the number of 4Kbytes sub-regions required to cover the end of the executable in Region 2 (up to 8 maximum)
    * This number of sub-regions is the left-SHIFT value we apply to decide which regions are disabled.
    */
  if (region2_first_4kb_sub_region_disabled != 0U)
  {
    /*
      ...
      */
    region2_first_4kb_sub_region_disabled = ((region2_first_4kb_sub_region_disabled + 4095U) >> 12U);
  }
  else
  {
    /*
      ...
      *    SUPPORTED
      */
    region2_first_4kb_sub_region_disabled = 0U; /* Will result in 0xFF << 0 : disable all sub-regions */
  }
  /*
  ...
  if (region1_first_32kb_sub_region_disabled <= SFU_PROTECT_MPU_MAX_NB_SUBREG)
  {
    ...
    if (((region1_first_32kb_sub_region_disabled * 32768U) + (region2_first_4kb_sub_region_disabled * 4096U)) <
        sbsfu_code_size)
    {
    ...
    for (mpu_region_num = 0U; mpu_region_num < (sizeof(MpuAreas) / sizeof(SFU_MPU_InitTypeDef)); mpu_region_num++)
    {
      ...
      if (mpu_region_num == 1U)
      {
        ...
        MPU_InitStruct.SubRegionDisable = (uint8_t)(0xffU << region1_first_32kb_sub_region_disabled);
      }
      else if (mpu_region_num == 2U)
      {
        /* Region 2 (executable, up to 32kB with up to 8*4kB sub-regions) */
        MPU_InitStruct.SubRegionDisable = (uint8_t)(0xffU << region2_first_4kb_sub_region_disabled);
        /* The region start address is the 16k-aligned address before the end address of the executable area */
        MPU_InitStruct.BaseAddress = (SFU_ROM_ADDR_END  & ~32767U); /* 32k LSBs set to 0 */
      }
      else
      {
...
SFU_ErrorStatus SFU_LL_SECU_SetProtectionIWDG(void)
{
  ...
  /* Set counter reload value to obtain 15 sec. IWDG TimeOut.
  ...
  So Set Reload Counter Value = (15 * uwLsiFreq) / 128 */
