...
(KEY IDbased APIs) that are executed inside a protected and isolated environment. User application keys are stored in
the protected and isolated environment. They can be updated in a secure way (authenticity check, data decryption and
data integrity check).

STSAFE-A1xx is a tamper-resistant secure element (HW Common Criteria EAL5+ certified) used to host X509 certificates
and keys, and to perform verifications that are used for firmware image authentication during Secure Boot and Secure
...
@par Directory contents
   ...
   - 2_Images_SBSFU/SBSFU/App/sfu_scheme_x509_mbedtls_ecdsa_verify.c Custom ecdsa_verify implementation for mbedtls (hands off
                                                             to stsafe via PKCS11)
   ...
   - 2_Images_SBSFU/SBSFU/App/sfu_scheme_x509_embedded_certs.h SBSFU embedded certificates to replace STSAFE in case
...
@par Hardware and Software environment
   ...
   - This example has been tested with B-L4S5I-IOT01A board and can be easily tailored to any other supported device and
     development board.
