...
SE_ErrorStatus SE_LL_FLASH_EXT_Decrypt_Init(SE_FwRawHeaderTypeDef *pxSE_Metadata)
{
  ...
  OTFDEC_RegionConfigTypeDef OTFDEC_Config = {0U};
  uint8_t key[16U];
  uint8_t key_swapped[16U];
  uint32_t i = 0U;
  uint32_t slot_number = 0U;

  /* Identify the slot number */
  if (memcmp(pxSE_Metadata->SFUMagic, SFUM_1, strlen(SFUM_1)) == 0U)
  {
    slot_number = 0U;
    SE_ReadKey_1(key);
  }
#if (SFU_NB_MAX_ACTIVE_IMAGE > 1U)
  else if (memcmp(pxSE_Metadata->SFUMagic, SFUM_2, strlen(SFUM_2)) == 0U)
  {
    slot_number = 1U;
    SE_ReadKey_2(key);
  }
  ...
  else if (memcmp(pxSE_Metadata->SFUMagic, SFUM_3, strlen(SFUM_3)) == 0U)
  {
    slot_number = 2U;
    SE_ReadKey_3(key);
  }
  ...
  if ((e_ret_status == SE_SUCCESS)
      && (HAL_OTFDEC_RegionSetMode(&OtfdecHandle, (OTFDEC_REGION1 + slot_number),
  ...
  for (i = 0U; i < 16U; i++)
  {
    key_swapped[i] = key[15U - i];
  }

  /* Clean-up RAM stacked key variable */
  (void)memcpy(key, (void const *)(SE_STARTUP_REGION_ROM_START + (SysTick->VAL % 0xFFFU)), sizeof(key));

  /* Set OTFDEC Key */
  if ((e_ret_status == SE_SUCCESS)
      && (HAL_OTFDEC_RegionSetKey(&OtfdecHandle, (OTFDEC_REGION1 + slot_number), (uint32_t *)key_swapped) != HAL_OK))
  {
    e_ret_status = SE_ERROR;
  }

  /* Clean-up RAM stacked key_swapped variable */
  (void)memcpy(key_swapped, (void const *)(SE_STARTUP_REGION_ROM_START + (SysTick->VAL % 0xFFFU)), sizeof(key_swapped));
  ...
  */
  ...
  OTFDEC_Config.StartAddress = SlotStartAdd[SLOT_ACTIVE_1 + slot_number] + SFU_IMG_IMAGE_OFFSET;
  OTFDEC_Config.EndAddress   = SlotEndAdd[SLOT_ACTIVE_1 + slot_number];
  OTFDEC_Config.Version      = 0x0;
  if ((e_ret_status == SE_SUCCESS)
      && (HAL_OTFDEC_RegionConfig(&OtfdecHandle, (OTFDEC_REGION1 + slot_number),  &OTFDEC_Config,
