...
SFU_ErrorStatus SFU_LL_SECU_CheckProtectionWRP(FLASH_OBProgramInitTypeDef *psFlashOptionBytes)
{
  ...
  i_ctrl = 0U;
  for (uint32_t i = SFU_PROTECT_WRP_LOADER_START_1; i <= SFU_PROTECT_WRP_LOADER_END_1; i++)
  {
    if ((psFlashOptionBytes->WRPSector & (OB_WRP_SECTOR_0TO3 << i)) == (OB_WRP_SECTOR_0TO3 << i))
    {
      i_ctrl++;   /*Protection applied */
  ...
  /* Double Check to avoid basic fault injection : no loop skipped */
  if (i_ctrl == (SFU_PROTECT_WRP_LOADER_END_1 - SFU_PROTECT_WRP_LOADER_START_1 + 1))
  {
    e_ret_status = SFU_SUCCESS;
  }
